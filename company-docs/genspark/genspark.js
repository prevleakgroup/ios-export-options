(function () {
  const defaults = {
    ...
    summary:
      'Qvedic now ships with a polished, high-trust visual system designed for modern mobility, reporting, and enterprise communication.',
    bullets: [
      'Refined brand positioning for B2B audiences',
      'Premium digital messaging for product launches',
      'Flexible content layer for future GenSpark automation'
    ]
  };
  ...
  async function loadContent() {
    ...
    if (endpoint && apiKey) {
      try {
        ...
        console.warn('GenSpark content request failed, using local fallback.', error);
      }
    }
  ...
  function renderContent(target, data) {
    ...
    eyebrow.textContent = 'GenSpark-ready content layer';
